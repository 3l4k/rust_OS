...
//To enable interrupt, add x86_64::instructions::interrupts::enable();a
// to the init below
...
extern "x86-interrupt" fn keyboard_interrupt_handler(_stack_frame: InterruptStackFrame) {
    ...
    // Define the type of the buffer (e.g., Vec<char> for character buffer)
    type BufferType = Vec<char>;

    // Create a static Mutex-wrapped buffer
    static BUFFER: Mutex<BufferType> = Mutex::new(BufferType::new());
        ...
        if let Some(key) = keyboard.process_keyevent(key_event) {
            match key {
                DecodedKey::Unicode(character) => {
                    ...
                    // if character == '\x08' {
                    //     // Backspace character
                    //     // Delete the last character by moving the cursor back and printing a space to overwrite it
                    //     let mut buffer = BUFFER.lock();
                    //     if let Some(_) = buffer.pop() {
                    //         // Clear the current character by moving the cursor back and printing a space
